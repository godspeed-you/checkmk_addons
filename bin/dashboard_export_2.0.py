...
class BaseVars():
    ...
    def __init__(self, user, board_name=None, legacy=False):
        ...
        legacy = True  # hack for 2.0 which does support legacy only
        if legacy:
            ...
            Path("{self.lib_local_path}/gui/plugins").mkdir(parents=True, exist_ok=True)
...
def parse_args(sysargv):
    ...
    parser.add_argument(
        ...
        "--dashboard",
        required=False,
        help="Dashboard to extract. If not provided, all dashboards will be extracted.")
#    hack for 2.0 which does support legacy only
#    parser.add_argument(
#        "-l",
#        "--legacy",
#        required=False,
...
#        )
    ...
    parser.add_argument(
        ...
        required=False,
        action="store_true",
        help="To see you new builtin dashboards, you need to restart the site apache. With this option, this is done automatically at the end."
        )
    parser.add_argument(
        "-v",
        "--verbose",
...
def write_definition(base):
    ...
    with open(f"{base.local_path}/{base.user}_dashboard_{base.board_name}.py", "w") as local_out:
...
def main():
    ...
    exporter = BaseVars(args.user, args.dashboard) # Checkmk 2.0 version. Everything is legacy
    setup_logging(args.verbose)
    logging.debug("Parsed arguments: %s", args)

    dashboard_file = Path(f'{exporter.path}/{exporter.user}/user_dashboards.mk')
    ...
    if args.restart_apache:
        subprocess.Popen(('omd', 'restart', 'apache'), stdout=subprocess.PIPE).communicate()
